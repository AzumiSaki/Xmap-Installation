...
```bat
-4 -M icmp_echo -R 100 -f saddr,clas,success,timestamp_str --output-filter="success = 1 && repeat = 0" -o alive_164.csv 192.168.164.0/24
```

### 扫 `192.168.74.0/24`

```bat
 -4 -M icmp_echo -R 100 -f saddr,clas,success,timestamp_str --output-filter="success = 1 && repeat = 0" -o alive_74.csv 192.168.74.0/24
```
...
```bat
-4 -M tcp_syn -p 80,443,8080,8443 -R 100 -f saddr,dport,clas,success,timestamp_str --output-filter="success = 1 && repeat = 0" -o web_ports_164.csv 192.168.164.0/24
```

### 管理端口

```bat
-4 -M tcp_syn -p 22,23,3389,5900,5985,5986 -R 100 -f saddr,dport,clas,success,timestamp_str --output-filter="success = 1 && repeat = 0" -o admin_ports_164.csv 192.168.164.0/24
```

### 数据库端口

```bat
-4 -M tcp_syn -p 1433,1521,3306,5432,6379,27017 -R 100 -f saddr,dport,clas,success,timestamp_str --output-filter="success = 1 && repeat = 0" -o db_ports_164.csv 192.168.164.0/24
```

如果你要扫 `192.168.74.0/24`，把输出文件名和网段改成 `74` 版本就行。

## 6. 用 nmap 对活主机做确认（以下指令在nmap的工作文件夹里面使用）
